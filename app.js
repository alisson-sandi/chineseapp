...
function renderListen(){
  ...
  $("spk").onclick=()=>speak(hzText(c));
  const ch=$("ch");
  opts.forEach(o=>{ const b=document.createElement("button"); b.className="choice"; b.textContent=shortM(o.m);
    b.dataset.full=o.m; b.onclick=()=>handleChoice(b,o.m===c.m,ch,c.m); ch.appendChild(b); });
  $("controls").innerHTML="";
  speak(hzText(c));
}
...
function renderMeaning(){
  ...
  const ch=$("ch");
  opts.forEach(o=>{ const b=document.createElement("button"); b.className="choice"; b.textContent=shortM(o.m);
    b.dataset.full=o.m; b.style.fontSize="1rem"; b.onclick=()=>handleChoice(b,o.m===c.m,ch,c.m); ch.appendChild(b); });
  $("controls").innerHTML="";
}

function handleChoice(btn,ok,container,correctM){
  container.querySelectorAll(".choice").forEach(b=>{ b.style.pointerEvents="none";
    if((b.dataset.full||b.textContent)===correctM) b.classList.add("right");
...
/* short meaning for quiz buttons — strip parenthetical tone/grammar hints */
function shortM(m){ return m.replace(/\s*\([^)]*\)/g,"").trim(); }
