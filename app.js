...
/* ---------- audio: Google Translate TTS (zh-TW) with browser fallback ---------- */
let audioEl=null;
function speak(text){
  if(!text) return;
  const url="https://translate.google.com/translate_tts?ie=UTF-8&client=tw-ob&tl=zh-TW&q="+encodeURIComponent(text);
  try{
    if(!audioEl){ audioEl=new Audio(); audioEl.setAttribute("playsinline","true"); }
    audioEl.src=url;
    const pr=audioEl.play();
    if(pr&&pr.catch) pr.catch(()=>browserSpeak(text));
  }catch(e){ browserSpeak(text); }
}
function browserSpeak(text){
  try{ const u=new SpeechSynthesisUtterance(text); u.lang="zh-TW"; u.rate=0.8;
...
function renderCard(){
  ...
  $("card").innerHTML=`
    ...
    ${revealed?egHtml:""}
    ${revealed?`<button class="spk" id="spk">🔊</button>`:`<div class="hint">tap reveal to see & hear</div>`}`;
  if(revealed){ $("spk").onclick=()=>speak(hzText(c)); }
  if(!revealed){
